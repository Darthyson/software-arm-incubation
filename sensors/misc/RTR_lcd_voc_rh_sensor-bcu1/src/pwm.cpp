...
void initPWM(void) {
	...
	timer16_0.matchMode(TIMER_MATCH_MAT2, CLEAR);  // clear the output of PIO2_6 to 1 when the timer matches MAT1
	timer16_0.match(TIMER_MATCH_MAT2, 5);      // match MAT1 when the timer reaches this value
	...
	timer16_0.matchMode(TIMER_MATCH_MAT3, RESET | INTERRUPT);
	timer16_0.match(TIMER_MATCH_MAT3, 10);     // match MAT3 after 1000 milliseconds
...
uint8_t setPWM(uint8_t level){
	if(level == 0){
		...
		timer16_0.match(TIMER_MATCH_MAT2, 10-level);      // match MAT1 when the timer reaches this value
	}
	return true;
}
