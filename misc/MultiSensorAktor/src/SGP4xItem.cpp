...
void SGP4xItem::Loop(uint32_t now, int updatedObjectNo)
{
	...
	{
		temp = comObjects->objectReadFloat(firstComIndex + 2) * 0.01f;
		hum = comObjects->objectReadFloat(firstComIndex + 3) * 0.01f;
	}
...
/*			if (sgp4x.measureRawSignal(hum, temp, temp != -100 && hum != -100) == SGP4xResult::success)
			{
				BCU->objectWrite(firstComIndex + 1, sgp4x.getVocIndexValue());
				BCU->objectWrite(firstComIndex + 4, sgp4x.getNoxIndexValue());
			}*/
